...
public class SesionInfanteDE {
    ...
    private void inicializar() {
        ...
        listaInfantesDE = new ListaDE();
        //LLenado de la bds
        listaInfantesDE.adicionarNodo(new Infante("Carlitos", (short) 1, (byte) 2, true,
                controlLocalidades.getCiudades().get(0).getNombre()));
        listaInfantesDE.adicionarNodo(new Infante("Juanita", (short) 2, (byte) 3, false,
                controlLocalidades.getCiudades().get(3).getNombre()));
        listaInfantesDE.adicionarNodo(new Infante("Martina", (short) 3, (byte) 1, false,
                controlLocalidades.getCiudades().get(1).getNombre()));
        listaInfantesDE.adicionarNodoAlInicio(new Infante("Mariana", (short) 4, (byte) 5, false,
                controlLocalidades.getCiudades().get(2).getNombre()));
        ayudante = listaInfantesDE.getCabeza();
        infante = ayudante.getDato();
        //Me llena el objeto List para la tabla
        listadoInfantes = listaInfantesDE.obtenerListaInfantes();
        pintarLista();
    }
    ...
    public ListaDE getListaInfantes() {
        return listaInfantesDE;
    }

    public void setListaInfantes(ListaDE listaInfantes) {
        this.listaInfantesDE = listaInfantes;
    }
    ...
    public void guardarInfante() {
        ...
        infante.setCodigo((short) (listaInfantesDE.contarNodos() + 1));
        if (alInicio.compareTo("1") == 0) {
            listaInfantesDE.adicionarNodoAlInicio(infante);
        } else {
            listaInfantesDE.adicionarNodo(infante);
        }
    ...
    public void irPrimero() {
        if (listaInfantesDE.getCabeza() != null) {
            ayudante = listaInfantesDE.getCabeza();
            infante = ayudante.getDato();
    ...
    public void pintarLista() {
        ...
        if (listaInfantesDE.getCabeza() != null) {
            //llamo a mi ayudante
            NodoDE temp = listaInfantesDE.getCabeza();
            int posX = 2;
            int posY = 2;
    ...
    public void eliminarInfante() {
        if (codigoEliminar > 0) {
            ...
            try {
                listaInfantesDE.eliminarInfante(codigoEliminar);
                irPrimero();
